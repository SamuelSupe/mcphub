...
Use the existing `auth.issuer`. Register a browser OAuth client with authorization code + PKCE S256 and the exact callback `https://admin.example.com/auth/callback`. A public client is supported by default; optionally set `admin.client_secret_env` for `client_secret_basic` or `client_secret_post`.
...
Relative database paths resolve from the YAML directory. Keep the existing SQLite path and key to reuse existing data; this change requires no SQLite schema conversion. `mode: local` remains the default, with no login and a numeric loopback-only listener that must never be published through a proxy.
...
Back up the database and retain `MCPHUB_CONFIG_KEY` separately. Changing the driver does not migrate existing data; tool groups and OpenAPI imports have no YAML representation. Multi-instance coordination, cross-database migration tooling, centralized session revocation and interactive third-party backend authorization are out of scope. Signing out does not revoke issued JWTs; access revocation depends on short-lived tokens, refresh authorization and identity-provider policy.
