...
SQLite 和单实例 PostgreSQL 使用 schema **7**；启动自动迁移，旧二进制不可写入升级后的库。授权请求和一次性 code 在进程内，重启需要重新开始未完成的登录；持久化会话和签名密钥保留。登录、目录同步入口仍应纳入已有 HTTPS 代理的请求大小与速率保护。会话最长 8 小时不等于身份源实时在线校验；全局登出、SCIM、SAML、多身份源选择和多实例一致性不属于本次实现。
